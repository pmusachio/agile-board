...
**Update 2026-07-06 — real usability bug found live.** Once Paulo got the backend actually working, he reported the panel's text was nearly unreadable. Root cause: the panel had a hardcoded `background:white` with no explicit `color`, so it inherited `body`'s themed text color — in dark mode that's `--text-primary: #e0e0e0` (light gray, meant for a dark background), landing as light-gray-on-white. Fixed by using this project's own theme variables (`--bg-secondary`/`--text-primary`/`--bg-input`) instead of a hardcoded color, matching every other themed element on the board. Verified in a browser preview in both themes: dark mode now renders `#16213e` background with `#e0e0e0` text (correct contrast), light mode renders white background with `#212121` text — both legible, no regression.
