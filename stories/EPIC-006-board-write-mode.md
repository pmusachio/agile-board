...
**Live data-loss bug found and fixed (2026-07-04):** a real drag-and-drop test on the live board (3 cards, EPIC-004/TASK-051/TASK-062) revealed that saveTask() wrote task.description/subtasks/notes from the in-memory task object even when the card had never been opened (storyToTask() defaults those fields to empty, only lazy-loaded on click). A pure status-changing drag still triggers autoSave(), so the empty unloaded body silently overwrote each file's real Description/Notes/Subtasks — status changed correctly, content was wiped. Fixed in saveTask() (21-write.js): when task._bodyLoaded is false, the body is now carried through byte-for-byte from the fresh GET instead of being rebuilt from the (empty) in-memory task. Verified via a round-trip test across all 40 stories on both the loaded and unloaded save paths (0 mismatches). The 3 affected files' wiped content was recovered from git history; their legitimate status changes were kept. Separately noticed (not fixed here, tracked as a follow-up): parseChecklist()'s regex only captures a checklist item's first line, so a subtask/AC item with soft-wrapped continuation lines would be silently truncated to one line if saved via the write-mode edit modal — the one instance of this in the repo (TASK-065) was reflowed to a single line defensively.
